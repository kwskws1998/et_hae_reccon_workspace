...
check_file "FINAL_PRETRAIN_CSV" "$FINAL_PRETRAIN_CSV"
check_file "FINAL_FINETUNE_CSV" "$FINAL_FINETUNE_CSV"
...
Set DATA_ROOT if the files are under the standard layout:
  export DATA_ROOT=/workspace/et_hae_reccon_workspace/data
...
Or set explicit paths:
  ...
  export FINAL_PRETRAIN_CSV=/path/to/final_pretrain_trt_scaled.csv
  export FINAL_FINETUNE_CSV=/path/to/final_finetune_trt_scaled.csv

To search a cloud box:
  find /workspace -type f \( -name 'provo.csv' -o -name 'train_and_valid.csv' -o -name 'iitb_sa1_sa2_cmcl_scaled.csv' -o -name 'final_pretrain_trt_scaled.csv' -o -name 'final_finetune_trt_scaled.csv' \) -print
EOF
  exit 1
fi
